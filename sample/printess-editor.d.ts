...
export interface iBcTheme {
  mobile: iBcPanel[],
  desktop: iBcPanel[],
  breakpoint: number,
  logoUrl: string,
  logoUrlInvers: string,
  animation: "bars" | "none" = "bars";
  primary: iBcColor
  text: iBcColor
  name: string
  css: string,
...
export type iExternalButton = {
  location: "button-bar" | "image-upload",
  label: string,
  icon?: iconName,
...
export interface iPrintessComponent {
  ...
  propertyChangedBySystem(propertyId: string)
...
export interface printessAttachParameters {
  ...
  /**
   * indicates taht new Panel-UI (BCUI) and forces printess to act different in certain ways
...
export interface iPanelUi {
  receiveMessage: (api: any, topic: externalMessageTopic, data: Record<string, any>) => void,
  refreshPriceDisplay: (api: any, info: iExternalPriceDisplay) => void,
}
...
export interface iPrintessApi {
   ...
   * Throws error if handlers are not currently attached or if scope is not initialize
   */
  detachAllHandlers();
  ...
  * Throws error if handlers are not currently detached.
  */
  attachAllHandlers();
  ...
  loadTemplateAndFormFields(
    ...
    formFieldProperties?: iFormFieldProperty[] | null): Promise<void>
   ...
   */
  fireSelectionChangeCallback(): Promise<void>;
   ...
   * ```
   */
  async transformSelection(position: {
    left?: number,
    top?: number,
    width?: number,
    height?: number,
  ...
  */
   ...
   */
  getFirstLevelImageButtons(p: iExternalProperty): Array<iMobileUIButton>
 ...
 */
  ...
  */
   ...
   * @param textStyleMode 
   */
  setTextStyleProperty(propertyId: string, name: "font" | "lineHeight" | "color" | "size" | "hAlign" | "vAlign" | "pStyle", value: string, textStyleMode?: textStyleModeEnum): Promise<void>;
   ...
   */
  assingImageByNameToSelection(imageName: string, placement: "fit" | "fill" | null): Promise<boolean>
  ...
  */
  ...
  resizePrintessExact(immediate: boolean, focusSelection: boolean, size: iRect, focusFormFieldId?: string, alwaysCenterSpread: boolean = false): Promise<void>
   ...
   * @param snippetUrl The Url of the snippet
   */
  insertGroupSnippet(snippetUrl: string, targetSplitterBoxId?: string, autoAssignToSelectionIfPossible: boolean = false): Promise<void>;
  ...
  /**
   * If the current selection should show text-frame-styles
   */
 ...
 */
  ...
  /**
   * Tells ui if it should display step nav buttons and avoid add-to-basket before last step
   */
  hidePreviousNextStepButtons();
   ...
   * @param zoom overrides the frames zoom settings for all devices
   * @param offset helps to get to the step before last step (offset = 1)
   */
  gotoLastStep(zoom?: "frame" | "spread", offset: number = 0): Promise<void>
...
export type iExternalFFUiType = "color" | "font" | "label" | "number" | "table" | "textbox" | "text-area" | "select-list" | "select-list+info" | "tab-list" | "image-list" | "color-list" | "image-id" | "checkbox";
...
export interface iExternalTextStyle {
  ...
  allows: Array<"insertFormFields" | "forceValidation" | "expressions" | "removeIfDefault" | "strokeWidth" | "clearOnFocus" | "offensive" | "content" | "mandatory" | "color" | "stroke" | "font" | "size" | "lineHeight" | "tracking" | "baselineShift" | "horizontalAlignment" | "verticalAlignment" | "padding" | "styles" | "bullet" | "indent" | "paragraphSpacing" | "baselineGrid" | "handWriting" | "letterGenerator">;
  pStyle: string;
}
export interface iExternalValidation {
  ...
  htmlInputType: "text" | "number" | "datetime";
  maxNumber: number;
}
...
export interface iExternalError {
  boxIds: Array<string>,
  pinnedDocId?: string,
  errorCode: "emptyError" | "preflight" | "rowIndexLessThanZero" | "invalidDayValue" | "imageResolutionLow" | "imageMissing" | "imageStillUploading" | "imageCouldNotUpload" | "textMissing" | "notChecked" | "characterMissing" | "maxCharsExceeded" | "offensiveLanguageDetected" | "regExpNotMatching" | "textOverflow" | "noLayoutSnippetSelected" | "invalidNumber" | "missingEventText" | "emptyBookPage",
...
export type MergeResource = "images" | "fonts" | "colors" | "snippet-lists" | "form-fields" | "new-form-fields" | "form-field-list-data" | "styles" | "layer-matrix";
...
export interface iExternalImageFrameInfos {
  ...
  conversion?: undefined | "fc-swap" | "bg-rm" | "upscale",
